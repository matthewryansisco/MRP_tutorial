...
temp = merge(puma_level_avgs, puma_zipcode_crosswalk, by.x="PUMA", by.y="puma12", all.x=T)
head(temp)

# Fill in missing afact values
# This will make it so zips across multiple pumas will equally weight the pumas where weights are missing
temp$afact[is.na(temp$afact)] <- 1
temp$afact <- as.numeric(temp$afact)

zip_level_avgs <- temp %>% group_by(zcta5) %>% summarize(commute = weighted.mean(commute, afact),
                                                         urban_transport = weighted.mean(urban_transport, afact),
                                                         only_english = weighted.mean(only_english, afact))
head(zip_level_avgs)

survey <- merge(survey, zip_level_avgs, by.x="live_zipcode", by.y="zcta5", all.x=T)  # Just a few cannot be matched  
names(survey)
